...
if conf['model'] == "ml2": ### Multi-level model
    model_1 = MLNet2(num_node_features, num_edge_features, num_graph_features, conf['hidden'], conf['dropout'],
                   conf['num_tasks'], conf['depth'], conf['graph_level_feature'])
    model_2 = MLNet2(num_node_features, num_edge_features, num_graph_features, conf['hidden'], conf['dropout'],
                   conf['num_tasks'], conf['depth'], conf['graph_level_feature'])
elif conf['model'] == "ml3": ### ablation studey: w/o subgraph-level
    model_1 = MLNet3(num_node_features, num_edge_features, num_graph_features, conf['hidden'], conf['dropout'],
                   conf['num_tasks'], conf['depth'], conf['graph_level_feature'])
    model_2 = MLNet3(num_node_features, num_edge_features, num_graph_features, conf['hidden'], conf['dropout'],
                   conf['num_tasks'], conf['depth'], conf['graph_level_feature'])
else:
...
if conf['task_type'] == "classification":
    run_classification(train_dataset, val_dataset, test_dataset, model_1, model_2, conf['num_tasks'], conf['epochs'], conf['batch_size'], conf['vt_batch_size'], conf['lr'], conf['lr_decay_factor'], conf['lr_decay_step_size'], conf['weight_decay'], conf['early_stopping'], conf['loss'], conf['metric'], args.log_dir, args.save_dir, args.evaluate, args)
elif conf['task_type'] == "regression":
    run_regression(train_dataset, val_dataset, test_dataset, model_1, model_2, conf['num_tasks'], conf['epochs'], conf['batch_size'], conf['vt_batch_size'], conf['lr'], conf['lr_decay_factor'], conf['lr_decay_step_size'], conf['weight_decay'], conf['early_stopping'], conf['metric'], args.log_dir, args.save_dir, args.evaluate)
else:
